    ...
    <el-row>
      ...
      <el-col :span="4">
        <div style="display: flex;justify-content:center" v-show="user.hasLogin">
          <div>
            <el-link :underline="false" type="danger" style="margin-top: 20px">消息</el-link>
          </div>
          ...
          <el-link v-if="!user.hasLogin">
            <router-link :to="{name: 'LoginPage'}">点击登陆</router-link>
          </el-link>
        </div>
        <div v-show="!user.hasLogin">
          <el-button type="danger" style="margin-top:10px" @click="$router.push('/login')" v-show="!hasLogin">登录
          </el-button>
          <el-button type="danger" style="margin-top:10px" @click="$router.push('/register')" v-show="!hasLogin">注册
          </el-button>
        </div>
      </el-col>
    </el-row>
